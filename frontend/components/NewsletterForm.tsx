    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch("http://localhost:8080/newsletter/subscribe", {
                ...
                body: JSON.stringify({
                    email,
                    source: variant === 'blog-post' ? 'blog_post_cta' : (variant === 'sidebar' ? 'sidebar' : 'main_form')
                }),
            });
        ...
        return (
            <div className={`${getVariantStyles(variant).container} ${className}`}>
                <div className="text-center">
                    ...
                    <h3 className={`${variant === 'blog-post' ? 'font-gilroy font-bold text-2xl text-black' : 'font-cutive font-normal text-2xl text-primary-900 !text-primary-900'} mb-3`}>
                        🎉 You&apos;re In!
                    </h3>
                    <p className={`${variant === 'blog-post' ? 'font-gilroy text-black' : 'font-body text-primary-900 !text-primary-900'} mb-4 font-light`}>
                        {variant === 'blog-post' ?
                            "Welcome to our community! You'll receive our latest insights and design tips every week." :
                            successMessage
                        }
                    </p>
                    <button
                        ...
                        className={`${variant === 'blog-post' ? 'text-[#236b7c] font-gilroy' : 'text-primary-900 font-body'} font-medium hover:text-neutral-600 transition-colors`}
                    >
        ...
        <div className={`${styles.container} ${className}`}>
            {variant === 'blog-post' && (
                <div className="text-center mb-8">
                    <h3 className="font-gilroy font-bold text-2xl sm:text-3xl text-black mb-4">
            ...
            <form onSubmit={handleSubmit} className={styles.form}>
                <div className={styles.inputGroup}>
                    ...
                    >
                        {isLoading ? (
                            ...
                            variant === 'blog-post' ? 'Join the Community' : 'Subscribe'
                        )}
                    </button>
                </div>
                ...
                {error && (
                    <div className="mt-3 p-3 bg-neutral-50 border border-neutral-200">
                        <p className={`text-sm ${variant === 'blog-post' ? 'text-black' : 'text-primary-900'}`}>{error}</p>
                    </div>
                )}
            </form>
...
function getVariantStyles(variant: string) {
    const styles = {
        default: {
            ...
            input: "w-full px-6 py-4 font-body text-primary-900 bg-white border-2 border-neutral-200 focus:outline-none focus:border-primary-900 placeholder-neutral-400 text-lg transition-all duration-300",
        ...
        inline: {
            container: "bg-white border border-neutral-200 p-8 my-12",
            form: "mb-4",
            inputGroup: "flex flex-col sm:flex-row gap-3 max-w-md mx-auto",
            input: "flex-1 pl-12 pr-4 py-3 font-body text-primary-900 bg-white border border-neutral-200 focus:outline-none focus:ring-2 focus:ring-primary-900 focus:border-primary-900 placeholder-neutral-400 transition-all duration-300",
            button: "bg-primary-900 text-white px-8 py-4 font-medium text-sm tracking-wide hover:bg-neutral-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed",
        ...
        sidebar: {
            ...
            input: "w-full px-4 py-3 font-body text-primary-900 bg-neutral-50 border border-neutral-200 focus:outline-none focus:ring-2 focus:ring-primary-900 focus:border-primary-900 placeholder-neutral-400 text-sm transition-all duration-300",
            button: "bg-primary-900 text-white px-8 py-4 font-medium text-sm tracking-wide hover:bg-neutral-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed",
            trustIndicators: ""
        },
        'blog-post': {
            ...
            input: "flex-1 px-4 py-4 font-gilroy text-black bg-white border-2 border-neutral-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#236b7c] focus:border-[#236b7c] placeholder-neutral-400 text-lg transition-all duration-300 shadow-sm",
